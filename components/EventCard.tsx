...
export default function EventCard({ eventId }: { eventId: Id<"events"> }) {
  ...
  return (
    <div
      ...
      className={`group bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-purple-100 cursor-pointer overflow-hidden relative transform hover:-translate-y-1 ${
    ...
    >
      ...
      <div className={`p-7 ${imageUrl ? "relative" : ""}`}>
        <div className="flex justify-between items-start">
          <div>
            <div className="flex flex-col items-start gap-2.5">
              {isEventOwner && (
                <span className="inline-flex items-center gap-1.5 bg-blue-500/90 text-white px-3 py-1 rounded-full text-xs font-medium shadow-sm">
                  <StarIcon className="w-3.5 h-3.5" />
                  Your Event
                </span>
              )}
              <h2 className="text-2xl font-bold text-gray-900 group-hover:text-purple-900 transition-colors duration-200">{event.name}</h2>
            </div>
            {isPastEvent && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 mt-2.5">
                Past Event
              </span>
            )}
          </div>
          ...
          <div className="flex flex-col items-end gap-2.5 ml-4">
            <span
              className={`px-4 py-2 font-semibold rounded-full shadow-sm ${
                isPastEvent
                  ? "bg-gray-50 text-gray-500"
                  : "bg-green-50 text-green-700 group-hover:bg-green-100 transition-colors duration-200"
              }`}
            >
              £{event.price.toFixed(2)}
            </span>
            {availability.purchasedCount >= availability.totalTickets && (
              <span className="px-4 py-1.5 bg-red-50 text-red-700 font-semibold rounded-full text-sm shadow-sm group-hover:bg-red-100 transition-colors duration-200">
                Sold Out
              </span>
            )}
          </div>
